...
float matchSpeakerAmplitude(float drillAmp, float freq) {
  ...
  for (int iter = 0; iter < 15; iter++) {
    targetGain = testGain;
    delay(300);
    measuredAmp = measureAtFrequency(freq, 10);
    ...
    if (testGain < 500.0f) testGain = 1000.0f;
...
void runCalibration() {
  ...
  // --- Step 1: Mic read (speaker off) ---
  outputEnabled = false;
  delay(1000);  // ensure silence
  ...
  FFT.compute(FFT_FORWARD);
  FFT.complexToMagnitude();
  for (int i = 1; i < BUFFER_SIZE / 2; i += 2) {
    float f = (float)i * SAMPLE_RATE / BUFFER_SIZE;
    Serial.printf("BEFORE,%.0f,%.3f\n", f, vReal[i]);
  }

  // --- Step 2: Find dominant frequency and amplitude ---
  float ampBefore = 0.0f;
  float detectedFreq = detectDominantFrequency(ampBefore);
  toneFreq = detectedFreq;
  computePhaseIncrement();

  // --- Step 3: Start continuous playback ---
  outputEnabled = true;
  delay(500);

  // --- Step 4: Match amplitude ---
  ...
  for (float ph = -M_PI; ph <= M_PI; ph += PHASE_STEP_COARSE) {
    ...
    Serial.printf("SWEEP,%.2f,%.4f\n", ph, amp);
    if (amp < bestAmp) { bestAmp = amp; bestPh = ph; }
  }
  for (float ph = bestPh - 0.2f; ph <= bestPh + 0.2f; ph += PHASE_STEP_FINE) {
    phaseOffset = ph;
    delay(DWELL_MS);
    float amp = measureAtFrequency(toneFreq, 3);
    Serial.printf("SWEEP,%.2f,%.4f\n", ph, amp);
  ...
  for (int i = 1; i < BUFFER_SIZE / 2; i += 2) {
    float f = (float)i * SAMPLE_RATE / BUFFER_SIZE;
    Serial.printf("AFTER,%.0f,%.3f\n", f, vReal[i]);
  }

  // --- Step 9: Print results for Python parser ---
  Serial.printf("RESULT,Before,%.4f\n", ampBefore);
  Serial.printf("RESULT,After,%.4f\n", ampAfter);
