...
#define DWELL_MS           1200            // was 300: give speaker+mic time to settle
...
float matchSpeakerAmplitude(float drillAmp, float freq) {
  ...
  for (int iter = 0; iter < 10; iter++) {
    ...
    if (ratio > 0.9f && ratio < 1.1f) break;  // within ±10% tolerance

    if (ratio > 1.1f) testGain *= 0.85f;  // too loud
    else testGain *= 1.15f;               // too quiet
  ...
  outputEnabled = false;
  Serial.printf("Matched Gain: %.1f (speaker ≈ drill)\n", testGain);
  return testGain;
}
